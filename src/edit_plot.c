...
static NamedParameterType S_MEASURED_VARIABLE_ID  = { "PL Measured Variable Id", PT_STRING };
							...
							if ((param_p = EasyCreateAndAddStringParameterToParameterSet (data_p, param_set_p, group_p, S_STUDY_ID.npt_type, S_STUDY_ID.npt_name_s, "Study ID", "The Study ID", study_id_s, PL_ALL)) != NULL)
								{
									...
									if ((param_p = EasyCreateAndAddStringParameterToParameterSet (data_p, param_set_p, group_p, S_MEASURED_VARIABLE_ID.npt_type, S_MEASURED_VARIABLE_ID.npt_name_s, "Measured Variable ID", "The URL of the Measured Variable to add a phenotype for", NULL, PL_ALL)) != NULL)
										{
											if ((param_p = EasyCreateAndAddStringParameterToParameterSet (data_p, param_set_p, group_p, S_PHENOTYPE_VALUE.npt_type, S_PHENOTYPE_VALUE.npt_name_s, "Phenotype", "The observed phenotypic value", NULL, PL_ALL)) != NULL)
												{
													if ((param_p = EasyCreateAndAddTimeParameterToParameterSet (data_p, param_set_p, group_p, S_PHENOTYPE_DATE.npt_name_s, "Date", "The date when the observation happened", NULL, PL_ALL)) != NULL)
														{
															StandardRow *row_p = NULL;

															if (active_plot_p)
																{
																	if ((active_plot_p -> pl_rows_p) && (active_plot_p -> pl_rows_p -> ll_size > 0))
																		{
																			RowNode *row_node_p = (RowNode *) (active_plot_p -> pl_rows_p -> ll_head_p);

																			if (row_node_p -> rn_row_p -> ro_type == RT_STANDARD)
																				{
																					row_p = (StandardRow *) (row_node_p -> rn_row_p);
																				}
																		}
																}

															if (AddExistingPhenotypeParameters  (param_set_p, row_p, dfw_data_p))
																{
																	success_flag = true;
																}
														...
														{
															PrintErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, "Failed to add %s parameter", S_PHENOTYPE_DATE.npt_name_s);
														}
										...
										{
											PrintErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, "Failed to add %s parameter", S_MEASURED_VARIABLE_ID.npt_name_s);
										}
