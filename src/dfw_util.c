							...
							if (BSON_APPEND_OID (query_p, MONGO_ID_S, &oid))
								{
									...
									if (results_p)
													...
													if (num_results == 1)
														{
															...
															if (!result_p)
																{
																	PrintJSONToErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, res_p, "failed to create object for id \"%s\"", object_id_s);
																}

														}		/* if (num_results == 1) */
													else
														{
															PrintJSONToErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, results_p, "" SIZET_FMT " results when searching for object_id_s with id \"%s\"", object_id_s);
														}
										...
										{
											PrintErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, "Failed to get results searching for object_id_s with id \"%s\"", object_id_s);
										}

								}		/* if (BSON_APPEND_OID (query_p, MONGO_ID_S, &oid)) */
							else
								{
									PrintErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, "Failed to create query for object_id_s with id \"%s\"", object_id_s);
								}
