	...
	if (PrepareSaveData (& (gene_bank_p -> gb_id_p), &selector_p))
		{
			...
			if (gene_bank_json_p)
				{
					if (SaveAndBackupMongoDataWithTimestamp (data_p -> dftsd_mongo_p, gene_bank_json_p, data_p -> dftsd_collection_ss [DFTD_GENE_BANK], data_p -> dftsd_backup_collection_ss [DFTD_GENE_BANK], DFT_BACKUPS_ID_KEY_S, selector_p, DFT_TIMESTAMP_S))
						{
							success_flag = true;
						}
					else
