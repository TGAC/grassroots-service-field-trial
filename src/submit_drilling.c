...
static ParameterSet *GetDrillingSubmissionServiceParameters (Service *service_p, DataResource *resource_p, User *user_p);
...
static void ReleaseDrillingSubmissionServiceParameters (Service *service_p, ParameterSet *params_p);

static ServiceJobSet *RunDrillingSubmissionService (Service *service_p, ParameterSet *param_set_p, User *user_p, ProvidersStateTable *providers_p);


static bool CloseDrillingSubmissionService (Service *service_p);

static ServiceMetadata *GetDrillingSubmissionServiceMetadata (Service *service_p);
...
Service *GetDrillingSubmissionService (GrassrootsServer *grassroots_p)
{
			...
			if (data_p)
				{
					if (InitialiseService (service_p,
														 ...
														 NULL,
														 GetDrillingSubmissionServiceParameters,
														 GetDrillingSubmissionServiceParameterTypesForNamedParameters,
														 ReleaseDrillingSubmissionServiceParameters,
														 CloseDrillingSubmissionService,
														 NULL,
														 false,
														 SY_SYNCHRONOUS,
														 (ServiceData *) data_p,
														 GetDrillingSubmissionServiceMetadata,
														 NULL,
														 grassroots_p))
						{
...
static const char *GetDrillingSubmissionServiceName (Service * UNUSED_PARAM (service_p))
{
	return "Submit Field Trial Drilling";
}
...
static bool GetDrillingSubmissionServiceParameterTypesForNamedParameters (struct Service *service_p, const char *param_name_s, ParameterType *pt_p)
{
	return GetSubmissionDrillingParameterTypeForNamedParameter (param_name_s, pt_p);
}



static ParameterSet *GetDrillingSubmissionServiceParameters (Service *service_p, DataResource * UNUSED_PARAM (resource_p), User * UNUSED_PARAM (user_p))
{
	...
	if (params_p)
		{
			...
			if (AddSubmissionDrillingParams (data_p, params_p))
				{
					return params_p;
				}
			else
		...
		{
			PrintErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, "Failed to allocate %s ParameterSet", GetDrillingSubmissionServiceName (service_p));
		}
...
static void ReleaseDrillingSubmissionServiceParameters (Service * UNUSED_PARAM (service_p), ParameterSet *params_p)
{
	FreeParameterSet (params_p);
}
...
static ServiceJobSet *RunDrillingSubmissionService (Service *service_p, ParameterSet *param_set_p, User * UNUSED_PARAM (user_p), ProvidersStateTable * UNUSED_PARAM (providers_p))
{
	FieldTrialServiceData *data_p = (FieldTrialServiceData *) (service_p -> se_data_p);

	service_p -> se_jobs_p = AllocateSimpleServiceJobSet (service_p, NULL, "Submit Drilling");
	...
	if (service_p -> se_jobs_p)
		{
			...
			if (!RunForSubmissionDrillingParams (data_p, param_set_p, job_p))
				{
