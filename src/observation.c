	...
	if ((!IsStringEmpty (raw_value_s)) || (!IsStringEmpty (corrected_value_s)))
		{
			...
			if (start_date_p)
				{
					copied_start_date_p = DuplicateTime (start_date_p);

					if (!copied_start_date_p)
						{
							PrintErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, "Failed to copy start date");
							success_flag = false;
						}
				}
			...
			if (success_flag)
				{
					...
					if (end_date_p)
						{
							copied_start_date_p = DuplicateTime (end_date_p);

							if (!copied_start_date_p)
								{
									PrintErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, "Failed to end date");
									success_flag = false;
								}
						}
	...
	if (CreateValidDateFromJSON (observation_json_p, OB_START_DATE_S, &start_date_p))
		{
			...
			if (CreateValidDateFromJSON (observation_json_p, OB_END_DATE_S, &end_date_p))
													...
													if (! ((IsStringEmpty (raw_value_s)) && (IsStringEmpty (corrected_value_s))))
														{
															...
															observation_p = AllocateObservation (id_p, start_date_p, end_date_p, phenotype_p, raw_value_s, corrected_value_s, growth_stage_s, method_s, instrument_p, nature);
				...
				{
					PrintJSONToErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, observation_json_p, "Failed to create date from \"%s\"", OB_END_DATE_S);
				}
