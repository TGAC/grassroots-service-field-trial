	...
	if (instrument_p -> in_id_p)
		{
			...
			if (instrument_json_p)
				{
					success_flag = SaveAndBackupMongoDataWithTimestamp (data_p -> dftsd_mongo_p, instrument_json_p, data_p -> dftsd_collection_ss [DFTD_INSTRUMENT], data_p -> dftsd_backup_collection_ss [DFTD_INSTRUMENT], DFT_BACKUPS_ID_KEY_S, selector_p, DFT_TIMESTAMP_S);
