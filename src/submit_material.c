...
static ParameterSet *GetMaterialSubmissionServiceParameters (Service *service_p, DataResource *resource_p, User *user_p);
...
static ServiceJobSet *RunMaterialSubmissionService (Service *service_p, ParameterSet *param_set_p, User *user_p, ProvidersStateTable *providers_p);


static bool CloseMaterialSubmissionService (Service *service_p);

static ServiceMetadata *GetMaterialSubmissionServiceMetadata (Service *service_p);
			...
			if (data_p)
				{
					if (InitialiseService (service_p,
														 ...
														 NULL,
														 GetMaterialSubmissionServiceParameters,
														 GetMaterialSubmissionServiceParameterTypesForNamedParameters,
														 ReleaseMaterialSubmissionServiceParameters,
														 CloseMaterialSubmissionService,
														 NULL,
														 false,
														 SY_SYNCHRONOUS,
														 (ServiceData *) data_p,
...
{
	return DFT_GROUP_ALIAS_PREFIX_S SERVICE_GROUP_ALIAS_SEPARATOR "submit_material";
}
...
static ParameterSet *GetMaterialSubmissionServiceParameters (Service *service_p, DataResource * UNUSED_PARAM (resource_p), User * UNUSED_PARAM (user_p))
{
	...
	if (params_p)
		...
		{
			PrintErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, "Failed to allocate %s ParameterSet", GetMaterialSubmissionServiceName (service_p));
		}
...
static void ReleaseMaterialSubmissionServiceParameters (Service * UNUSED_PARAM (service_p), ParameterSet *params_p)
{
	FreeParameterSet (params_p);
}
...
static ServiceJobSet *RunMaterialSubmissionService (Service *service_p, ParameterSet *param_set_p, User * UNUSED_PARAM (user_p), ProvidersStateTable * UNUSED_PARAM (providers_p))
{
	FieldTrialServiceData *data_p = (FieldTrialServiceData *) (service_p -> se_data_p);

	service_p -> se_jobs_p = AllocateSimpleServiceJobSet (service_p, NULL, "Submit Material");
